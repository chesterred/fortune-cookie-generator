...
export function randomMessage() {
    const text = messages[Math.floor(Math.random() * messages.length)].message;

    const count = 6; // 生成的数字数量
    const max = 59; // 数字的最大值
    let numbers = [];
    ...
    return `${text} Lucky Numbers: ${numbers.join('-')}`;
}

export default function ImageSwitcher() {
    // 用于切换图像的状态
    const [showImageA, setShowImageA] = useState(true);
    const imageAUrl = '/closed-fortune-cookie.png'; // 替换为图像A的路径

    return (
        <>
            {/* <Navbar /> */}
            <article className='format lg:format-lg  mx-auto mt-10'>
                <h1 className='text-center'>Fortune Cookie Generator</h1>
                {/*图片和按钮 */}
                    ...
                    <div className='flex -mt-10 max-w-md'>
                        {showImageA ? (
                            <Image
                                src={imageAUrl}
                                alt="Fortune Cookie"
                                width={512} // 适当设置图像的宽度和高度
                                height={512}
                                onClick={() => setShowImageA(!showImageA)}
                                quality={80}
                                className='cursor-pointer hover:rotate-12 max-w-full '
                            />
                        ) : (
                            <div className='flex max-w-lg'  onClick={() => setShowImageA(!showImageA)}>
                                <CanvasWithText backgroundImage="opened-fortune-cookie.png" text={randomMessage()} width={1000} height={1000} />
                            </div>

                        )}
                    </div>
                    <div className="flex flex-col  gap-2">
                        <button type="button" className="text-white bg-gradient-to-br from-pink-500 to-orange-400 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-pink-200 dark:focus:ring-pink-800 font-medium rounded-lg text-lg px-5 py-2.5 text-center me-2 mb-2 "
                            onClick={() => setShowImageA(!showImageA)}
                        >{showImageA ? 'Generate your fortune cookie' : 'Generate another'}
                        </button>
                        <Link href="/custom-fortune-cookie">
                            <button type="button" className="text-white bg-gradient-to-br from-purple-600 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-lg px-5 py-2.5 text-center me-2 mb-2"
                            >Custom fortune cookie message
                            </button>
                        </Link>
                ...
                <h2>Hello</h2>
                <p>I am Chestered, and I am glad to tell you that I made this small tool, the Fortune Cookie Generator, which can generate a fortune cookie with a message and a lucky number. The number is composed of six two-digit numbers. Also, there is a custom fortune cookie message generator. You can type in your message and get your customized fortune cookie.</p>

<p>Why did I make this? Last week, I went to a Chinese restaurant and got a fortune cookie. The message of this fortune cookie, &quot;Fall seven times and stand up eight,&quot; helped me a lot because sometimes I thought I should give up on my entrepreneurship. So, I thought I should make a Fortune Cookie Generator to bring fun and inspiration.</p>

<h2>The Tech Behind the Magic</h2>

<p>I built this generator using Next.js, and for the visual enthusiasts, the images are crafted through the Canvas API. This tech choice ensures everything happens in your local environment – your data stays with you, as privacy is my top priority. The quotes? They&apos;re from a variety of great minds across the globe, offering a rich tapestry of wisdom.</p>

<h2>Your Privacy Matters</h2>

<p>As I mentioned, your privacy is non-negotiable. Everything from the generation of your personalized fortune cookie to the lucky numbers (a set of six 2-digit numbers, by the way) happens on your device. No data goes to the servers or the internet. It&apos;s all about keeping it personal and secure.</p>

<h2>What&apos;s Next?</h2>

<p>The future looks exciting! I&apos;m planning to integrate AI technology into the website. Imagine a fortune cookie generator that uses ChatGPT&apos;s analytical prowess to tailor fortunes based on your input – a personalized touch to make each message even more special.</p>

<h2>A Bit About Me</h2>

<p>As for me, I&apos;m a full-stack programmer with experience in various fields, including social media promotion on platforms like Google and Facebook. If you&apos;re curious about my other projects or want to connect, check out my GitHub: <a href="https://github.com/chesterred">https://github.com/chesterred</a>.</p>
